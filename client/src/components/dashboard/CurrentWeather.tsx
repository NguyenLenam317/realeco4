    ...
    return (
      <Card className={`shadow-md ${className}`}>
        <CardContent className="p-6">
          ...
          <div className="flex flex-col md:flex-row items-center w-full min-w-0">
            <div className="text-center md:text-left md:mr-6 mb-4 md:mb-0 flex-grow min-w-0">
        ...
        <div className="mt-6">
          ...
          <div className="flex overflow-x-auto py-2 justify-between">
            {(() => {
              ...
              if (data.hourly?.time?.length) {
                for (let i = 0; i < data.hourly.time.length; i++) {
                  try {
                    ...
                    if (hour > currentHour || time.getDate() > currentDate.getDate()) {
              ...
              return hourlyItems.map((item, index) => (
                <div key={index} className="flex flex-col items-center mx-auto min-w-[60px] px-2">
                  <p className="text-sm text-neutral-500">{item.time}</p>
                  <span className="material-icons text-lg my-2">{item.icon}</span>
